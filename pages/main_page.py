...
class MainPage(BasePage):
    ...
    def get_font_size(self, item, element=None):
        ...
        if item == FIRST_SAFETY:
            ...
            element = self.get_footer_license()
        elif item == EMAIL_TEXT:
            element = self.get_footer_email()
        elif item == YEAR_COOPERATION:
            element = self.get_footer_cooperation()
        return element

    @pytest.mark.parametrize('button', BUTTONS)
    def get_button_is_visible(self, button, element=None):
        """Проверка видимости кнопки в хедере"""
